...
/**
 * Writes a plain text `.txt` file of all parsed systems.
 *
 * @param {Array<object>} systems - List of parsed systems from readLine()
 * @param {string} outputPath - Where to save the output file
 * @param {(msg: string) => void} onMessage - Optional progress callback
 */
export async function systemFormatter(systems, outputPath, onMessage = () => { }) {
    const content = systems.map(formatAsSystem).join('');
    await fs.writeFile(outputPath, content, 'utf8');
    onMessage(`📝 System data written to ${outputPath}`);
}